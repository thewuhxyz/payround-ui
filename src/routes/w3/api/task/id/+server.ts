...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	// 	.data?.user_id;
	const taskResult = await supabase.from('task').select('*').eq('task_key', taskkey).single();
	const task = taskResult.data;

	return new Response(JSON.stringify({ task }));
	// const payroundClient = payroundAdmin()
	// const taskKey = new PublicKey(taskkey);
	...
	// const tx = await payroundAdmin().formatTxDataFor(thread)
	...
	// return new Response (JSON.stringify ({ nextRun, decodedStatus, threadBalance, tx, ...task}))
	// ;
};
