...
export const load: PageServerLoad = async ({ locals, params, parent }) => {
	...
	const threadBalance =
		((await payroundClient.connection.getBalance(thread)) - task.rent!) / PayroundClient.MSOL;
	...
	if (task!.cron) {
		nextRun = parser.parseExpression(task!.schedule, {utc: true}).next().toDate().getTime();
	} else {
